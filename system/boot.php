...
try {
    ...
    $err = '<strong>' . htmlspecialchars(get_class($e)) . ':</strong> '
        . htmlspecialchars($e->getMessage())
        . '<br><small>File: ' . htmlspecialchars($e->getFile()) . ':' . $e->getLine() . '</small>'
        . '<br><pre style="white-space:pre-wrap;">' . htmlspecialchars($e->getTraceAsString()) . '</pre>';
    if (!isset($ui)) {
        echo '<h2>Database Error (boot stage)</h2>' . $err;
        die();
    }
    $ui->assign("error_message", $err);
    $ui->assign("error_title", "Database Error");
...
try {
    ...
    $err = '<strong>' . htmlspecialchars(get_class($e)) . ':</strong> '
        . htmlspecialchars($e->getMessage())
        . '<br><small>File: ' . htmlspecialchars($e->getFile()) . ':' . $e->getLine() . '</small>'
        . '<br><small>Route: ' . htmlspecialchars(isset($_GET['_route']) ? $_GET['_route'] : '(default)') . '</small>'
        . '<br><pre style="white-space:pre-wrap;">' . htmlspecialchars($e->getTraceAsString()) . '</pre>';
    $ui->assign("error_message", $err);
    $ui->assign("error_title", "Ray System Error");
    if (empty($_SESSION['aid'])) {
        $ui->display('customer/error.tpl');
    } else {
        $ui->display('admin/error.tpl');

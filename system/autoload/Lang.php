...
class Lang
{
    ...
    public static function getNotifText($key)
    {
        ...
        if (isset($_notifmsg[$key]) && $_notifmsg[$key] !== '' && $_notifmsg[$key] !== null) {
            return $_notifmsg[$key];
        }
        if (isset($_notifmsg_default[$key])) {
    ...
    /**
     * Resolve a notification template for a plan/customer type.
     * Prefers *_pppoe / *_hotspot keys when present, else falls back to $baseKey.
     *
     * @param string $baseKey e.g. welcome_message
     * @param array|null $plan
     * @param array|null $customer
     */
    public static function getNotifTextForPlan($baseKey, $plan = null, $customer = null)
    {
        ...
        } elseif (is_array($customer) && !empty($customer['service_type'])) {
            $type = strtoupper(trim((string) $customer['service_type']));
        }
        if ($type === 'PPPOE') {
            $text = self::getNotifText($baseKey . '_pppoe');
            if (is_string($text) && trim($text) !== '') {
                return $text;
            }
        } elseif ($type === 'HOTSPOT') {
            $text = self::getNotifText($baseKey . '_hotspot');
            if (is_string($text) && trim($text) !== '') {
                return $text;
            }
        }
        return self::getNotifText($baseKey);
    }

...
class Admin
{
    private static function getSessionTimeoutDurationSeconds($config)
    {
        $minutes = isset($config['session_timeout_duration']) ? (int) $config['session_timeout_duration'] : 0;
        if ($minutes <= 0) {
            $minutes = 60;
        }
        return $minutes * 60;
    ...
    public static function getID()
    {
        ...
        if (!empty($isApi) && !empty($_SESSION['aid'])) {
            return (int) $_SESSION['aid'];
        }

        $enable_session_timeout = $config['enable_session_timeout'] == 1;
        $session_timeout_duration = self::getSessionTimeoutDurationSeconds($config);
        if ($isApi) {

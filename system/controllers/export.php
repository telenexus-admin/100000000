...
switch ($action) {
    ...
    case 'print-by-date':
        ...
        if (is_array($rts) && count($rts) > 0) {
            $query->where_in('routers', $rts);
        }
        if (is_array($plns) && count($plns) > 0) {
    ...
    case 'pdf-by-date':
        ...
        if ($x) {
            ...
            $mpdf->SetWatermarkText($config['CompanyName']);
...
EOF;
            ...
            $mpdf->Output('antiqua_reports_'.date('Ymd_His') . '.pdf', 'D');
        } else {
            echo 'No Data';
        }

...
function expenditure_dashboard()
{
    ...
    $currency = !empty($config['currency_code']) ? $config['currency_code'] : 'Ksh.';

    expenditure_ensure_tables();

    $todayExpenses = (float) (ORM::for_table('tbl_expenditures')
        ->where('expense_date', $today)
        ->sum('amount') ?: 0);

    $thisMonthExpenses = (float) (ORM::for_table('tbl_expenditures')
        ->where_gte('expense_date', $thisMonthStart)
        ->where_lte('expense_date', $thisMonthEnd)
        ->sum('amount') ?: 0);

    $lastMonthExpenses = (float) (ORM::for_table('tbl_expenditures')
        ->where_gte('expense_date', $lastMonthStart)
        ->where_lte('expense_date', $lastMonthEnd)
        ->sum('amount') ?: 0);

    $thisYearExpenses = (float) (ORM::for_table('tbl_expenditures')
        ->where_gte('expense_date', $thisYearStart)
        ->where_lte('expense_date', $thisYearEnd)
        ->sum('amount') ?: 0);
    ...
    $recentExpenses = ORM::for_table('tbl_expenditures')
        ...
        ->order_by_desc('tbl_expenditures.expense_date')
        ->order_by_desc('tbl_expenditures.id')
        ->limit(10)
        ->find_many();
    ...
    $topCategories = ORM::for_table('tbl_expenditures')
        ...
        ->where_gte('tbl_expenditures.expense_date', $thisMonthStart)
        ->where_lte('tbl_expenditures.expense_date', $thisMonthEnd)
        ->group_by('tbl_expenditures.category_id')
        ->order_by_desc('total_amount')
        ->limit(5)
        ->find_many();

    $ui->assign('currency_code', $currency);
    $ui->assign('todayExpenses', $todayExpenses);
    $ui->assign('thisMonthExpenses', $thisMonthExpenses);
    $ui->assign('lastMonthExpenses', $lastMonthExpenses);
    $ui->assign('thisYearExpenses', $thisYearExpenses);
    $ui->assign('recentExpenses', $recentExpenses);
    $ui->assign('topCategories', $topCategories);
    $ui->assign('_title', 'Expenditure Dashboard');

    $ui->display('expenditure_dashboard_modern.tpl');
}

/**
 * Create expenditure tables/categories if missing (safe no-op when present).
 */
function expenditure_ensure_tables()
{
    static $done = false;
    if ($done) {
        return;
    ...
    $done = true;
    try {
        ORM::raw_execute("CREATE TABLE IF NOT EXISTS `tbl_expenditure_categories` (
        ...
        ORM::raw_execute("CREATE TABLE IF NOT EXISTS `tbl_expenditures` (
            `id` int(11) NOT NULL AUTO_INCREMENT,
        ...
        $count = (int) ORM::for_table('tbl_expenditure_categories')->count();
        if ($count === 0) {
            $defaults = [
                ['Equipment', 'Network equipment, servers, hardware'],
                ['Internet Bandwidth', 'Upstream internet connectivity costs'],
                ...
                ['Transport', 'Vehicle fuel, maintenance, travel'],
                ['Professional Services', 'Legal, accounting, consulting'],
            ];
            foreach ($defaults as $d) {
                $c = ORM::for_table('tbl_expenditure_categories')->create();
                $c->name = $d[0];
                $c->description = $d[1];
                $c->save();
            }
        ...
        // keep page usable even if ensure fails
    }
}
...
function expenditure_add()
{
    ...
    $ui->assign('currency_code', !empty($config['currency_code']) ? $config['currency_code'] : 'Ksh.');
...
function expenditure_save()
{
    ...
    $id = _post('id', _req('id'));
    $description = trim((string) _post('description', _req('description')));
    $amount = _post('amount', _req('amount'));
    $category_id = _post('category_id', _req('category_id'));
    $expense_date = trim((string) _post('expense_date', _req('expense_date')));
    $receipt_number = trim((string) _post('receipt_number', _req('receipt_number')));
    $vendor = trim((string) _post('vendor', _req('vendor')));
    $notes = trim((string) _post('notes', _req('notes')));
    ...
    if ($id) {
        ...
        $expense->created_by = isset($admin['id']) ? (int) $admin['id'] : 0;
        $expense->created_at = date('Y-m-d H:i:s');
    }

    $expense->description = $description;
    $expense->amount = round((float) $amount, 2);
    $expense->category_id = ($category_id !== '' && $category_id !== null) ? (int) $category_id : null;
    ...
    try {
        ...
        r2(U . 'plugin/expenditure&action=add', 'e', 'Could not save expenditure: ' . $e->getMessage());
    }

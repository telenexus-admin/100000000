        ...
        function apiGetText(){
            ...
            if(el != undefined){
                $.get(el.getAttribute('api-get-text'), function(data) {
                    // Never inject full HTML error pages into table cells
                    if (typeof data === 'string' && (data.indexOf('<!DOCTYPE') !== -1 || data.indexOf('<html') !== -1 || data.indexOf('Oops!') !== -1)) {
                        el.innerHTML = '<button type="button" class="btn btn-danger btn-xs btn-conn-status" disabled>Error</button>';
                    } else {
                        el.innerHTML = data;
                    }
                    posAttApi++;

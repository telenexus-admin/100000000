<?php
/**
 * Pamnet Safe Notifications — deploys a hardened cron_reminder.php once.
 * Upload via Plugin Manager as a zip containing this file.
 */

if (!defined('ROOTPATH') && !isset($root_path)) {
    // loaded outside app — ignore
    return;
}

$__pamnetCron = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'cron_reminder.php';
$__pamnetMarker = 'PAMNET_SAFE_REMINDER_V3';

if (file_exists($__pamnetCron)) {
    $__cur = @file_get_contents($__pamnetCron);
    if (is_string($__cur) && strpos($__cur, $__pamnetMarker) !== false) {
        return;
    }
}
...
/**
 * Pick SMS/WA channel for plan type (Orange Theme + legacy).
 */
$resolveVia = function ($type, $kind) use ($config) {
    $kind = strtolower((string) $kind);
    $isPppoe = strtoupper((string) $type) === 'PPPOE';
    $keys = $isPppoe
        ? ["user_notification_{$kind}_pppoe", "user_notification_{$kind}"]
        : ["user_notification_{$kind}_hotspot", "user_notification_{$kind}"];
    foreach ($keys as $key) {
...
$ok = @file_put_contents($__pamnetCron, $__safeCron);
if ($ok === false) {
    // Try alternate path styles
    $alt = realpath(dirname(__DIR__)) . DIRECTORY_SEPARATOR . 'cron_reminder.php';
    $ok = @file_put_contents($alt, $__safeCron);
}

if ($ok !== false) {
    // one-time log if possible
    try {
        if (class_exists('ORM')) {
            // no-op; file written
        }

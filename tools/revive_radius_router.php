...
try {
    ...
    ORM::configure('driver_options', $driverOptions);
    ...
    ORM::configure('driver_options', $driverOptions, 'radius');
...
try {
    ...
    $stmt = $radiusDb->prepare('SELECT * FROM nas WHERE nasname = ? LIMIT 1');
    $stmt->execute([$tunnelIp]);
    $byIp = $stmt->fetch(PDO::FETCH_ASSOC);

    $stmt = $radiusDb->prepare('SELECT * FROM nas WHERE shortname = ? LIMIT 1');
    ...
    if ($nas) {
        if ($hasRouters) {
            $stmt = $radiusDb->prepare("UPDATE nas SET nasname=?, shortname=?, type='other', ports=NULL, secret=?, server=NULL, community=NULL, description=?, routers=? WHERE id=?");
            $stmt->execute([$tunnelIp, $shortName, $secret, $description, (string) $router['name'], (int) $nas['id']]);
        } else {
            $stmt = $radiusDb->prepare("UPDATE nas SET nasname=?, shortname=?, type='other', ports=NULL, secret=?, server=NULL, community=NULL, description=? WHERE id=?");
            $stmt->execute([$tunnelIp, $shortName, $secret, $description, (int) $nas['id']]);
        }
    } else {
        if ($hasRouters) {
            $stmt = $radiusDb->prepare("INSERT INTO nas (nasname,shortname,type,ports,secret,server,community,description,routers) VALUES (?,?,'other',NULL,?,NULL,NULL,?,?)");
            $stmt->execute([$tunnelIp, $shortName, $secret, $description, (string) $router['name']]);
        } else {
            $stmt = $radiusDb->prepare("INSERT INTO nas (nasname,shortname,type,ports,secret,server,community,description) VALUES (?,?,'other',NULL,?,NULL,NULL,?)");
    ...
    if ($candidateIds) {
        ...
        $radiusSet = new PEAR2\Net\RouterOS\Request('/radius/set');
        $radiusSet->setArgument('numbers', $radiusId);
    } else {
        $radiusSet = new PEAR2\Net\RouterOS\Request('/radius/add');
    }

    $radiusSet->setArgument('address', $radiusAddress)
